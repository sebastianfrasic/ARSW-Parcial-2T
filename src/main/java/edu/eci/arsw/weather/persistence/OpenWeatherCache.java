...
public class OpenWeatherCache implements CacheInterface{
    ...
    public void removeFromCache(Weather weather) throws OpenWeatherPersistenceException {
        if (!isOnCache(weather.getCity())) {
            throw new OpenWeatherPersistenceException("This city is not on cache");
        }
        weatherArrayList.remove(weather);
    }

    public boolean isOnCache(String city) throws OpenWeatherPersistenceException {
